...
void CreateProcessAsTrustedInstaller(DWORD pid, wstring commandLine)
{
	...
	if ((hTIProcess = OpenProcess(
		...
		pid)) == nullptr)
	{
		MessageBoxA(NULL, "OpenProcessToken failed (TrustedInstaller.exe)", NULL, MB_OK | MB_ICONSTOP);
		...
		&hDupToken))
	{
		...
		MessageBoxA(NULL, "DuplicateTokenEx failed (TrustedInstaller.exe)", NULL, MB_OK | MB_ICONSTOP);
	...
	if (!CreateProcessWithTokenW(
		...
		&processInfo))
	{
		MessageBoxA(NULL, "CreateProcessWithTokenW FAILED!", NULL, MB_OK | MB_ICONSTOP);
