...
std::string ConvertTCHARToString(const TCHAR* tcharString)
{
	...
	std::string str;
	str.resize(length - 1);
	WideCharToMultiByte(CP_UTF8, 0, tcharString, -1, &str[0], length, nullptr, nullptr);
...
BOOL ListDeviceDrivers(HWND hWnd)
{
	...
	for (int j = 0; j < cDriver; j++)
	{
		if (GetDeviceDriverBaseName(drivers[j], szDriver, sizeof(szDriver)))
		{
			...
			std::string driverName = ConvertTCHARToString(szDriver);
			sDeviceDrivers.insert(driverName); // Insert the std::string into the set
		}
		else
		{
			printError(hWnd, _T("GetDeviceDriverBaseName"));
...
VOID printError(HWND hWnd, TCHAR* msg)
{
	...
	TCHAR szBuffer[kb(1)];
	_stprintf_s(szBuffer, _T("Failed With Error %d (%s)\n"), msg, eNum, sysMsg);
